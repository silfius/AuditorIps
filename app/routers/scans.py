        ...
        with db() as conn:
            ...
            if events:
                ...
                try:
                    ...
                    send_email("📡 Auditor IPs — Eventos de red", msg)
                except Exception:
                    pass
            ...
            if alert_msgs and cfg("discord_webhook", ""):
                for amsg in alert_msgs:
                    discord_notify(amsg)
            # Email para alertas programadas
            if alert_msgs:
                try:
                    from routers.config_api import send_email
                    for amsg in alert_msgs:
                        send_email("🔔 Alerta — Auditor IPs", amsg)
                except Exception:
                    pass
...
def api_scan_ai_report_by_id(report_id: int):
    """Devuelve el texto completo de un informe IA por ID."""
    try:
        with db() as conn:
            row = conn.execute(
                "SELECT * FROM scan_ai_reports WHERE id=?", (report_id,)
            ).fetchone()
        if not row:
            from fastapi.responses import JSONResponse
...
def api_scan_ai_analyze_now():
    """
    Genera un informe IA de las discrepancias actuales, directamente y de forma síncrona.
    No depende de la config de scan secundario — siempre funciona si hay IA configurada.
    """
    try:
        with db() as conn:
            disc_rows = conn.execute(
        ...
        if not report_text:
            ...
            if not provider or provider == "none":
                return JSONResponse({"ok": False, "error": "No hay proveedor IA configurado. Ve a Config → IA."})
            return JSONResponse({"ok": False, "error": "La IA no devolvió respuesta. Verifica la API key en Config → IA."})

        with _db_write_lock:
            with db() as conn:
                conn.execute(
                    "INSERT INTO scan_ai_reports (generated_at, report_text, discrepancy_count, source) VALUES (?,?,?,?)",
                    (utc_now_iso(), report_text, len(discs), "manual")
                )

        return JSONResponse({"ok": True, "report_text": report_text, "discrepancy_count": len(discs)})

    except Exception as e:
        import traceback
        return JSONResponse({"ok": False, "error": str(e), "detail": traceback.format_exc()[:500]})
